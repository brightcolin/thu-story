...
The [MIT License](LICENSE) applies only to original program and test code under `source-code/`, CI automation under `.github/workflows/`, and original software configuration files required to build, run, or test that code.

The README files, `AGENTS.md`, `CLAUDE.md`, `docs/`, `media/`, `game-build/`, project names, marks, and third-party materials are excluded from the MIT grant and are all rights reserved unless otherwise stated. See [NOTICE](NOTICE) for the precise scope.
